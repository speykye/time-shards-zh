...
*   “定金到底是哪天转的？”
*   “这张图是最终版还是草稿？”
*   **“报价 500 元，平台扣完税完我到底能拿多少？”**

**Time-Shards 就是一个专门用来记录这些“关键时刻”的电子记事本 + 算账工具。**
...
### 第零步：算清账目 (Calculator) —— **新功能！**
在正式接单前，先算清楚这笔钱到底值不值。
1.  在左侧侧边栏点击 **“🧮 费率计算器”**。
2.  **选择模式**：
    *   **正算 (税前 → 到手)**：输入你的报价，自动扣除平台费和税费，看看到手多少。
    *   **反算 (到手 → 税前)**：输入你想拿到手的钱，自动反推你应该报多少价。
3.  **调整参数**：自定义税率（如 6%）和平台抽成（如 5%）。
4.  **参考结果**：看着“预计到手”金额，决定是否接这个单子。
    *   *💡 提示：计算仅在本地进行，不会上传任何数据，刷新页面即重置。*
...
2.  点击 **“新建项目”**。
3.  起个名字，比如 **“狼人设定图 - 张三”**。
4.  （可选）在“项目简介”里写一句重点，例如：“3 个姿势，商用授权，10 月 1 日交稿”。
...
*   **计算器隐私**：费率计算器的所有数据仅在内存中运行，**绝不联网**，刷新页面即消失，绝对安全。
...
1.  **接单前（算账）**：
    *   打开 **“🧮 费率计算器”**。
    *   输入金主给的预算，看看扣完费到手多少。如果太少，直接拒绝或要求加价。
    *   *(心里有底，不被忽悠)*

2.  **接单时（定规矩）**：
    *   新建项目。
    *   创建一条 **“📨 信函”**，填好价格（可以是计算器算出的税前报价）、工期、修改次数、商用范围。
    *   发送给金主确认。
...
**Q: 费率计算器准吗？**
A: 它基于你输入的税率和平台费率进行数学计算，公式绝对准确。但实际扣费可能因平台政策微调（如最低手续费、阶梯税率等）而略有不同，**结果仅供参考**，请以平台最终账单为准。
